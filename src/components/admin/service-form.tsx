  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t("admin.services.deleteService")}</h3>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
          {t("admin.services.deleteServiceConfirm", { name: service.name })}
          {" "}
          {t("admin.services.deleteServiceNotes")}
        </p>
      ...
      <div className="mx-4 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">
            {isEdit ? t("admin.services.editService") : t("admin.services.addNewService")}
          </h2>
        ...
        {state.success && (
          <div className="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 dark:border-green-800 dark:bg-green-950/30 dark:text-green-400">
            ✅ {isEdit ? t("admin.services.serviceUpdated") : t("admin.services.serviceAdded")}
          </div>
        )}
        ...
        >
          {/* Name */}
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.serviceName")}</label>
            <input
              type="text" value={name} onChange={(e) => setName(e.target.value)}
              placeholder={t("admin.services.serviceNamePlaceholder")} required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.duration")}</label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.buffer")}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.price")}</label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.dpPercentage")}</label>
          ...
          {/* Description */}
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.description")}</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              rows={2} placeholder={t("admin.services.descriptionPlaceholder")}
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
            />
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
            <div>
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                {t("admin.services.onlineConsultation")}
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t("admin.services.onlineConsultationNotes")}
              </p>
            </div>
            <button
              type="button"
          ...
          {/* Color */}
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t("admin.services.color")}</label>
          ...
          >
            {isPending ? t("common.saving") : isEdit ? `💾 ${t("admin.services.updateService")}` : `💾 ${t("admin.services.saveService")}`}
          </button>
        </form>
      </div>
    </div>
